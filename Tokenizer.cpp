...
Tokenizer::Tokenizer(std::string& _in) :text(_in) {}

std::string Tokenizer::enumToString(Token::TokenType t) {
	switch (t)
	{
	case Token::MAP_INC:
		return "MAP-INC";
...
bool Tokenizer::hasMoreTokens(std::string in, std::string type) {
	bool result = false;
...
void fillProperties(Token& result, Token::TokenType tokenType, std::string paramName, std::string params) {
	result.type = tokenType;
	if (paramName == "Fail") result.type == Token::ERR;
	result.paramName = paramName;
	//bool isOnlyNums = isOnlyNumbers(params);
	if (params == "Fail") result.type == Token::ERR;
...
std::string findName(std::string& paramVal) {
	...
	Tokenizer tokanizer(paramVal);
	std::string paramValCopy = paramVal;

	std::string stringFromFileCopy = paramVal;

	Token token = tokanizer.getNextToken(paramVal, stringFromFileCopy);
	...
	while (!tokenStack.empty()) {
		...
		if (currentToken.moreTokens) {
			...
			if (!flag) {
				...
				Token tk = tokanizer.getNextToken(paramVal, stringFromFileCopy);
...
Token Tokenizer::getNextToken(std::string& in, std::string& inCopy) {
	Token result;
	...
	if (next == '<') {
		...
		if (isLike("MAP-INC", tagNm))
			fillProperties(result, Token::MAP_INC, paramName, params);
		else if (isLike("MAP-MLP", tagNm))
			fillProperties(result, Token::MAP_MLT, paramName, params);

		//Getting the Aggregate tags
		else if (isLike("AGG-SUM", tagNm))
			fillProperties(result, Token::AGG_SUM, paramName, params);
		else if (isLike("AGG-PRO", tagNm))
			fillProperties(result, Token::AGG_PRO, paramName, params);
		else if (isLike("AGG-AVG", tagNm))
			fillProperties(result, Token::AGG_AVG, paramName, params);
		else if (isLike("AGG-FST", tagNm))
			fillProperties(result, Token::AGG_FST, paramName, params);
		else if (isLike("AGG-LST", tagNm))
			fillProperties(result, Token::AGG_LST, paramName, params);

		//Getting the Sorting tags
		else if (isLike("SRT-REV", tagNm))
			fillProperties(result, Token::SRT_REV, paramName, params);
		else if (isLike("SRT-ORD", tagNm))
			fillProperties(result, Token::SRT_ORD, paramName, params);
		else if (isLike("SRT-SLC", tagNm))
			fillProperties(result, Token::SRT_SLC, paramName, params);
		else if (isLike("SRT-DST", tagNm))
			fillProperties(result, Token::SRT_DST, paramName, params);
...
std::string Tokenizer::doTasks(Tokenizer& tokanizer, Token& currentToken) {
	std::string resultCurrToken = "";
	...
	if (currentToken.type == Token::MAP_INC)
		...
		resultCurrToken = tokanizer.doSrtDst(currentToken);
	else if (currentToken.type == Token::LET_TAG)
		resultCurrToken = currentToken.params;

	return resultCurrToken;
}

std::string Tokenizer::doMapInc(Token& token) {
	std::string result = "";
...
std::string Tokenizer::doMapMlp(Token& token) {
...
std::string Tokenizer::doAggSum(Token& token) {
	std::vector<int> paramsAsInt = paramsInVector(token.params);
...
std::string Tokenizer::doAggPro(Token& token) {
	std::vector<int> paramsAsInts = paramsInVector(token.params);
	std::string tokenNString = token.paramName;
	int res = 1;
...
std::string Tokenizer::doAggAvg(Token& token) {
	std::vector<int> paramsAsInt = paramsInVector(token.params);
	int res = 0;
...
std::string Tokenizer::doAggFst(Token& token) {
	std::vector<int> paramsAsInt = paramsInVector(token.params);
...
std::string Tokenizer::doAggLst(Token& token) {
	std::vector<int> paramsAsInt = paramsInVector(token.params);
...
std::string Tokenizer::doSrtRev(Token& token) {
	std::string result = "";
...
std::string Tokenizer::doSrtOrd(Token& token) {
	std::string result = "";
	std::vector<int> paramsAsInt = paramsInVector(token.params);
...
std::string Tokenizer::doSrtSlc(Token& token) {
	std::vector<int> paramsAsInt = paramsInVector(token.params);
	std::string result = "";
...
std::string Tokenizer::doSrtDst(Token& token) {
	std::string result = "";
	std::vector<int> paramsAsInt = paramsInVector(token.params);
